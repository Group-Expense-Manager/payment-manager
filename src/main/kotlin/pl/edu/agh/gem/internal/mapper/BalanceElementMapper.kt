...
class BalanceElementMapper {
    fun mapToBalanceElement(
        userId: String,
        payment: Payment,
    ): BalanceElement? =
        when {
            userId.isPaymentParticipant(payment) ->
                BalanceElement(
    ...
    private fun String.isPaymentParticipant(payment: Payment) = setOf(payment.creatorId, payment.recipientId).contains(this)

    private fun getSignedValue(
        payment: Payment,
        userId: String,
    ): BigDecimal {
